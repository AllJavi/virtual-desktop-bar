...
Component {
    Rectangle {
        ...
        Text {
            ...
            opacity: {
                if (config.DesktopLabelsStyle == 5)
                    return 0.0;
	...
	Image {
            width: {
		if (!config.DesktopLabelsCustomIconSize) 
                    return (parent.width + 0.5 - 2 * config.DesktopButtonsSpacing);
                return (parent.width + 0.5 - 2 * config.DesktopButtonsSpacing) * (config.DesktopLabelsCustomIconSize / 100);
            }
		...
		if (!config.DesktopLabelsCustomIconSize) 
		    return (label.implicitHeight + 2 * config.DesktopButtonsVerticalMargin);
		return (label.implicitHeight + 2 * config.DesktopButtonsVerticalMargin) * (config.DesktopLabelsCustomIconSize / 100);
            }
	    y: (parent.height - height) / 2;
            mipmap: true
            fillMode: Image.PreserveAspectFit
	    source: {
		if (config.DesktopLabelsCustomIconPath == "")
		    return "/usr/share/virtual-desktop-bar/desktop-icons/desktop_" + number.toString() + ".svg"
		return config.DesktopLabelsCustomIconPath + "desktop_" + number.toString() + ".svg"
	    }
	    opacity: (config.DesktopLabelsStyle == 5) ? 1.0 : 0.0
        }
        ...
        function updateLabel() {
            label.text = Qt.binding(function() {
                ...
                if (config.DesktopLabelsStyle == 1 || config.DesktopLabelsStyle == 5) {
                    labelText = number + "";
